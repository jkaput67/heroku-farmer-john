...
class Portfolio_Gallery_Frontend_Scripts
{
    ...
    public function frontend_styles($id, $portfolio_view)
    {
        ...
        if ($portfolio_view == '7') {
            wp_register_style('portfolio-gallery-elastic-grid-css', plugins_url('../assets/style/elastic_grid.css', __FILE__));
            wp_enqueue_style('portfolio-gallery-elastic-grid-css');
        }
        if ($portfolio_view == '8') {
    ...
    public function frontend_scripts($id, $portfolio_view)
    {
        ...
        wp_register_script('portfolio-gallery-front-end-js-' . $view_slug, plugins_url('../assets/js/view-' . $view_slug . '.js', __FILE__), array('jquery'), '1.0.0', true);
        wp_enqueue_script('portfolio-gallery-front-end-js-' . $view_slug);
    ...
    public function localize_scripts($id)
    {
        ...
        $view_slug = portfolio_gallery_get_view_slag_by_id($id);
        $portfolio_obj = portfolio_gallery_get_portfolio_by_id($id);
        global $wpdb;
        ...
        if ($portfolio_param['portfolio_gallery_lightbox_type'] == 'old_type') {
            ...
            $lightbox_options['portfolio_gallery_lightbox_watermark_container_bg_color'] = 'rgba(' . $r . ',' . $g . ',' . $b . ',' . $titleopacity . ')';
        ...
        wp_localize_script('portfolio-custom-js', 'portfolio_lightbox_type', $portfolio_param['portfolio_gallery_lightbox_type']);
        wp_localize_script('portfolio-gallery-front-end-js-' . $view_slug, 'portfolio_param_obj', $portfolio_param);

        wp_localize_script('portfolio-gallery-front-end-js-' . $view_slug, 'portfolio_obj' . $id, $portfolio_obj);

        wp_localize_script('portfolio-gallery-front-end-js-' . $view_slug, 'images_obj_' . $id, $images_obj);
