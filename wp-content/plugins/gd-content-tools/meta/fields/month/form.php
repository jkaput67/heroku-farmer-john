...
if (!empty($this->settings['default'])) {
    $_year = substr($this->settings['default'], 0, 4);
    $_month = substr($this->settings['default'], 5, 2);
}
...
<table class="form-table">
    <tbody>
        ...
        <tr style="<?php echo $this->settings['custom'] ? '' : 'display: none;'; ?>">
            ...
            <td>
                <input class="widefat gdcet-third-field gdcet-datetime-month gdcet-field-property-settings-default" name="gdcet[field][fields][<?php echo $field_id; ?>][settings][default]" type="text" value="<?php echo esc_attr($_year.'-'.$_month); ?>" />
                <p class="description">
