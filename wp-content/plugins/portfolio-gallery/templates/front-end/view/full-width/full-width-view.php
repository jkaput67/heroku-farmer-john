	...
	<div id="huge_it_portfolio_options_and_filters_<?php echo esc_attr($portfolioID); ?>">
		...
		<?php if ( $portfolioShowSorting == "on" ) { ?>
			<div id="huge_it_portfolio_options_<?php echo esc_attr($portfolioID); ?>"
				...
				<ul  class=" clearfix sort-by-button-group" >
...
<?php } ?>
	<div id="huge_it_portfolio_container_<?php echo esc_attr($portfolioID); ?>"
	     ...
	     class="huge_it_portfolio_container super-list variable-sizes clearfix view-<?php echo esc_attr($view_slug); ?>" <?php if ( $portfolio_gallery_get_options["portfolio_gallery_ht_view3_sorting_float"] == "top" && $portfolio_gallery_get_options["portfolio_gallery_ht_view3_filtering_float"] == "top" ) {
		...
		foreach ( $images as $key => $row ) {
			...
			<div
				...
				<div class="left-block_<?php echo $portfolioID; ?>">
					<div class="main-image-block_<?php echo esc_attr($portfolioID); ?> add-H-relative">
						...
						if ( $row->image_url != ';' ) {
							switch ( portfolio_gallery_youtube_or_vimeo_portfolio( $imgurl[0] ) ) {
								case 'image':
									...
									?>
									<a href="<?php echo esc_url( $imgurl[0] ); ?>" <?php if ( $row->link_target == "on" ) {
                                        echo 'target="_blank"';
                                    } ?>
                                       ...
                                       title="<?php echo esc_attr( $row->name ); ?>" data-groupID="<?php echo esc_attr($group_key);?>"><img
											...
											id="wd-cl-img<?php echo $key; ?>" data-title=" <?php echo portfolio_gallery_get_image_title($imgurl[0]); ?>"
									...
									switch ( portfolio_gallery_youtube_or_vimeo_portfolio( $img ) ) {
										case 'image':
											...
											<li><a href="<?php echo esc_url( $img ); ?>"
                                                   ...
                                                   data-description=" <?php echo esc_attr( $row->description ); ?>"
                                                   data-groupID="<?php echo esc_attr($group_key);?>"><img alt="<?php echo esc_attr( $row->name ); ?>"
										...
										case 'youtube':
											...
											<li>
												<a href="https://www.youtube.com/embed/<?php echo esc_attr($videourl[0]); ?>"
                                                   data-description=" <?php echo esc_attr( $row->description ); ?>"
												   class="huge_it_portfolio_item pyoutube portfolio-group<?php echo esc_attr($group_key); ?>  add-H-relative"
												   title="<?php echo esc_attr( $row->name ); ?>" data-groupID="<?php echo esc_attr($group_key);?>">
													<img alt="<?php echo esc_attr($row->name); ?>"
														src="//img.youtube.com/vi/<?php echo esc_attr($videourl[0]); ?>/mqdefault.jpg">
													<div
