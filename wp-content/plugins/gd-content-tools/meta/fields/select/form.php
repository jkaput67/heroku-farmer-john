    ...
    <tbody>
        <tr>
            ...
            <td>
                ...
                ?>
                <p class="description">
                    <?php _e("If the source is set to 'Remote', display will be autmatically switched to 'Enhanced'.", "gd-content-tools"); ?>
                </p>
            </td>
        </tr>
        <tr>
        ...
        <tr class="gdcet-select-source gdcet-select-source-list <?php echo $this->settings['mode'] == 'normal' ? 'gdcet-select-mode-plain' : ''; ?>" style="<?php echo $this->settings['source'] == 'list' ? '' : 'display: none;'; ?>">
            ...
            <td>
                ...
                $_current_field_select_type = 'radio';
        ...
        <tr class="gdcet-select-source gdcet-select-source-remote" style="<?php echo $this->settings['source'] == 'remote' ? '' : 'display: none;'; ?>">
            <th scope="row"><?php _e("Remote Source", "gd-content-tools"); ?></th>
            <td>
                <?php

                    d4p_render_select($this->get_remote(), array('selected' => $this->settings['remote'], 'name' => 'gdcet[field][fields]['.$field_id.'][settings][remote]', 'class' => 'widefat gdcet-field-property-settings-remote'));

                ?>
            </td>
        </tr>
        <tr>
            <th scope="row"><?php _e("Hide 'Nothing Selected'", "gd-content-tools"); ?></th>
            <td>
                <input class="widefat gdcet-field-property-settings-hide_empty" name="gdcet[field][fields][<?php echo $field_id; ?>][settings][hide_empty]" type="checkbox"<?php echo $this->settings['hide_empty'] ? ' checked="checked"' : ''; ?> />
                <p class="description">
                    <?php _e("If enabled, the empty value added at the start of the list will not be included.", "gd-content-tools"); ?>
                </p>
            </td>
        </tr>
    </tbody>
