	...
	public function load_portfolio_page() {
		...
		switch ( $task ) {
			...
			case 'apply':
				...
				$b = $a ? wp_verify_nonce( $_REQUEST['hugeit_portfolio_apply_portfolio_nonce'], 'apply_portfolio_' . $id ) : false;
				$c = $a ? wp_verify_nonce( $_REQUEST['hugeit_portfolio_apply_portfolio_nonce'], 'remove_project_' . ( isset( $_GET['removeslide'] ) ? absint( $_GET['removeslide'] ) : '' ) ) : false;
...
GROUP BY " . $wpdb->prefix . "huge_itportfolio_images.portfolio_id ";
	...
	function save_portfolio_data( $id ) {
		...
		$portfolio_position     = in_array( $_POST["sl_position"], array(
			'on',
			'off'
		) ) ? $_POST["sl_position"] : 'on';
		...
		$wpdb->update(
			...
			array(
				...
				'sl_position'              => $portfolio_position
			),
			array( 'id' => $id )
		);
